...
class Agendamento:
    ...
    @staticmethod
    def buscar_horarios_livres(duracao_minutos, dias_limite=90):
        """
        Mapeia os próximos 3 meses (90 dias) e retorna as datas e horários livres
        baseado na configuração da clínica e duração exigida.
        """
        conn = get_db_connection()
        config = conn.execute("SELECT hora_abertura, hora_fechamento FROM configuracoes_clinica LIMIT 1").fetchone()
        hora_abertura = config['hora_abertura'] if config else 8
        hora_fechamento = config['hora_fechamento'] if config else 20
        
        agendamentos = conn.execute("""
            SELECT data_hora_inicio, data_hora_fim_previsto 
            FROM agendamentos 
            WHERE status NOT IN ('Cancelado') 
            AND data_hora_inicio >= date('now', 'localtime')
        """).fetchall()
        conn.close()
        ...
        for i in range(dias_limite):
            ...
            tempo_atual = datetime.datetime.combine(dia, datetime.time(hora_abertura, 0))
            fim_expediente = datetime.datetime.combine(dia, datetime.time(hora_fechamento, 0))
            
            while tempo_atual + datetime.timedelta(minutes=duracao_minutos) <= fim_expediente:
                fim_slot = tempo_atual + datetime.timedelta(minutes=duracao_minutos)
                conflito = False
                
                for ag in agendamentos:
                    inicio_ag = datetime.datetime.strptime(ag['data_hora_inicio'], "%Y-%m-%d %H:%M:%S")
                    fim_ag = datetime.datetime.strptime(ag['data_hora_fim_previsto'], "%Y-%m-%d %H:%M:%S")
                    
                    # Checagem de sobreposição na mesma sala
                    if (tempo_atual < fim_ag) and (fim_slot > inicio_ag):
                        conflito = True
                        break
                
                if not conflito:
                    horarios_livres.append(tempo_atual.strftime("%H:%M"))
                
                # Incrementos de 30 em 30 minutos na varredura da agenda
                tempo_atual += datetime.timedelta(minutes=30) 
                
            if horarios_livres:
                disponibilidade[str_dia] = horarios_livres
    ...
    def criar(cliente_id, profissional_id, servico_id, data_hora_inicio, observacoes=None):
        ...
        if not servico: 
            return None # Proteção contra serviço inexistente
